...
if __name__ == '__main__':
    ...
    # all_bn = np.array([1.0,0.6,0.5,0.4,0.3])# 2ms, 4ms, 10ms, 20ms, 100ms
    A = para_convert()
    pll_long_bn = np.array([2.0, 2.0, 1.5, 1.0, 0.4]) #1103
    ...
    gps_cn0_db = np.array([33.0, 29.8, 27.3, 23, 19, 17.5, 15, 13, 39.0, 39.0])  # enable 1103
    ...
    fll_bn = np.array([1.5])
    fll_sum_bn = np.array([[20], [16], [12.0]])
    pll_bn = np.array([[16], [12], [8], [5.0], [1.0]]) #1103
    ...
    pll_bn = np.array([[10], [8], [6.0], [3.5], [1.0]])
    ...
    pll_bn = np.array([[8], [6], [4], [3.0], [2.0]])
    fll3_bn = np.array([1.0])
    fll3_sum_bn = np.array([[12], [8], [4.0]])
    pll3_bn = np.array([[10], [4], [4], [3], [1.0]])
    all_bn = []
    # limit_cfg = [12, 100, 50, 16, 35, 22, 17, 12]
    limit_cfg = [12, 100, 50, 16, 35, 32, 22, 17]
    print("the low state:")
    result = A.cal(dll_sum_bn, pll_bn, pll_long_bn, pll3_bn, fll_sum_bn,fll3_sum_bn, dll_bn, fll_bn, fll3_bn, all_bn, mode, rank, limit_cfg, gps_cn0_db, smooth_fac_factor)
